...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<{ content: Array<{ type: string; text: string }> }> {
  switch (name) {
    case "archive_sessions": {
      ...
      // Find or create dataset once
      const allDs = await (await fetch(`${RAGFLOW_API}/api/v1/datasets`, { headers: { Authorization: `Bearer ${RAGFLOW_TOKEN}` } })).json()
      const allDatasets = Array.isArray(allDs.data) ? allDs.data : allDs.data?.docs ?? []
      let dsId = allDatasets.find((d: any) => d.name === datasetName)?.id
      if (!dsId) {
        const createRes = await fetch(`${RAGFLOW_API}/api/v1/datasets`, {
          method: "POST",
          headers: { Authorization: `Bearer ${RAGFLOW_TOKEN}`, "Content-Type": "application/json" },
          body: JSON.stringify({ name: datasetName }),
        })
        const createBody = await createRes.json()
        dsId = createBody.code === 0
          ? (Array.isArray(createBody.data) ? createBody.data[0]?.id : createBody.data?.id)
          : undefined
      }
      if (!dsId) return { content: [{ type: "text", text: "❌ Failed to find or create dataset" }] }
